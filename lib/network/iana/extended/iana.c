...
// { "www-http"                            , 80    , "udp"  },
  ...
  { "realsecure"                          , 2998  , "tcp"  },
  { "realsecure"                          , 2998  , "udp"  },
  { "remoteware-un"                       , 2999  , "tcp"  },
  { "remoteware-un"                       , 2999  , "udp"  },
  { "hbci"                                , 3000  , "tcp"  },
  { "hbci"                                , 3000  , "udp"  },
  ...
  { "datusorb"                            , 3282  , "tcp"  },
  { "datusorb"                            , 3282  , "udp"  },
  { "apple remote desktop (net assistant)", 3283  , "tcp"  },
  { "apple remote desktop (net assistant)", 3283  , "udp"  },
  { "4talk"                               , 3284  , "tcp"  },
  { "4talk"                               , 3284  , "udp"  },
  { "plato"                               , 3285  , "tcp"  },
  { "plato"                               , 3285  , "udp"  },
  ...
  { "ipr-dglt"                            , 3678  , "tcp"  },
  { "ipr-dglt"                            , 3678  , "udp"  },
  { "escale (newton dock)"                , 3679  , "tcp"  },
  { "escale (newton dock)"                , 3679  , "udp"  },
...
network_iana_protocolbyname(const char *name) {
  ...
  for (protocol = 0; protocol < NETWORK_IANA_PROTOCOLS_SIZE; protocol++)
    if (string_equals(network_iana_protocols[protocol].name, name, true))
...
network_iana_protocolbyport(uint16_t port) {
  size_t protocol, start = 0, end = NETWORK_IANA_PROTOCOLS_SIZE;

  for (;;) {
    if ((protocol = (start+end)/2) == NETWORK_IANA_PROTOCOLS_SIZE-1)
      break;

    if (network_iana_protocols[protocol].port == port)
      return &network_iana_protocols[protocol];
    else if (network_iana_protocols[protocol].port < port)
      start = protocol;
    else if (network_iana_protocols[protocol].port > port)
      end = protocol;
  }
